## AI Agent Guidance for `smg-copa-mattress`
...
- `tailwind.config.js`: extends Tailwind with a custom `copa-blue` palette.
...
## Build / Run / Lint
- Install dependencies from `smg-copa-mattress` root: `npm install`
...
- The current Tailwind setup uses a custom color key `copa-blue`; if classes do not appear, confirm `tailwind.config.js` and `src/index.css` custom utilities.
- This workspace is multi-project: do not edit `admin/` or `copa/` unless the task explicitly targets those directories.
